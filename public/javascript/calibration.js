...
function main() {
    ...
    var predPoint = (activity.getAppMode() !== "DEMO") ? true : false;
    ...
    /**
     * To enable the loading screen
     * This is a (fake) loading screen which makes the user wait for 1500 ms before calibration
     * Used to let the browser load the webgazer and draw the elements.
     * But with calibration version 2 (1 node display at a time), we don't need to make the user wait.
     */
    $("#dialog").dialog();
    setTimeout(function () {
        $("#dialog").dialog("close");
